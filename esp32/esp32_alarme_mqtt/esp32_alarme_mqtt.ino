...
// SMS via Twilio (conta trial gratuita)
// ### PREENCHA AQUI ### com os dados do seu console.twilio.com
const char* twilio_account_sid    = "AC4d9501bf4ca36f69db910b83b260c6fb";
const char* twilio_auth_token     = "f36f7b6acecb3a114925240762a9e9be";
const char* twilio_numero_origem  = "%2B18604922446";    // numero Twilio (From), ja com %2B no lugar do +
const char* twilio_numero_destino = "%2B5511991122803";  // seu celular verificado na conta trial (To)
...
// Codificacao percent-encoding generica, usada nas chamadas HTTPS
// para CallMeBot (WhatsApp) e Twilio (SMS).
...
// Terceiro canal de alerta: SMS via Twilio. O numero de destino precisa
// estar verificado no console da Twilio (limitacao da conta trial).
void enviarSMS(String mensagem) {
  ...
  if (https.begin(httpClient, url)) {
    ...
    String corpo = "To=" + String(twilio_numero_destino) +
                   "&From=" + String(twilio_numero_origem) +
                   "&Body=" + urlEncode(mensagem);

    int codigo = https.POST(corpo);
    Serial.print("Twilio HTTP: ");
    Serial.println(codigo);

    if (codigo != 201) {
      Serial.println(https.getString()); // ajuda a ver o motivo do erro no Serial Monitor
    }

    https.end();
  } else {
    Serial.println("Falha ao iniciar requisicao HTTPS para o Twilio.");
  }
}
